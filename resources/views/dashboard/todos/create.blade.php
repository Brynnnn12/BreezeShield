<x-layout.dashboard title="Tambah Todo">
    <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="flex items-center mb-6">
            <a href="{{ route('todos.index') }}" class="text-blue-600 hover:text-blue-700 mr-4">
                <i class="fas fa-arrow-left text-lg"></i>
            </a>
            <h3 class="text-lg font-semibold text-gray-800">Tambah Todo Baru</h3>
        </div>
        ...
        <form action="{{ route('todos.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" required
                    ...
                    placeholder="Masukkan deskripsi todo">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i>Simpan Todo
                </button>
            </div>
        </form>
    </div>
